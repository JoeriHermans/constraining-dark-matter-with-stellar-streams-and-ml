...
def plot_2d_profile_likelihood(ax, profile_likelihood, mass=None, age=None):
    ...
    im = ax.imshow(profile_likelihood.T + 1, norm=LogNorm(), alpha=.75, interpolation="bilinear", vmax=13, extent=extent, origin="lower", cmap=cmap)
    if mass is not None and age is not None:
        ax.scatter(mass, age, s=150, marker='*', c="#ff4747", alpha=1.0, zorder=10)
    ax.set_xlabel(r"$m_{\textsc{wdm}}$")
    ax.set_xticks([10, 20, 30, 40, 50])
...
def plot_2d_profile_likelihood(ax, profile_likelihood, mass=None, age=None):
    ...
    resolution = len(profile_likelihood)
    cmap = palettable.scientific.sequential.Oslo_3_r.mpl_colormap
    im = ax.imshow(profile_likelihood.T + 1, norm=LogNorm(), alpha=.75, interpolation="bilinear", extent=extent, origin="lower", cmap=cmap)
    if mass is not None and age is not None:
        ax.scatter(mass, age, s=150, marker='*', c="#ff4747", alpha=1.0, zorder=10)
    ax.set_xlabel(r"$m_{\textsc{wdm}}$")
    ax.set_xticks([10, 20, 30, 40, 50])
...
@torch.no_grad()
def compute_1d_pdf_abc(mock_path, resolution=default_resolution, bandwidth=2.5, new=False):
    ...
    if new:
        samples = np.load("out/mock/abc-new-" + suffix + "/samples.npy")[:, 0] # Masses only
    else:
...
@torch.no_grad()
def compute_2d_pdf_abc(mock_path, resolution=default_resolution, bandwidth=2.5, new=False):
    ...
    suffix = mock_path.split('/')[-1].split('-')[1]
    if new:
        samples = np.load("out/mock/abc-new-" + suffix + "/samples.npy")
    else:
