...
#SBATCH --export=ALL
#SBATCH --parsable
#SBATCH --job-name "STREAM_SIMULATE_TEST"
#SBATCH --output "logging/stream_simulate_test_%a.log"
#SBATCH --requeue
...
out=$DATADIR/test/$task_identifier
mkdir -p $out
...
if [ ! -f $out/densities.npy -o $PROJECT_FORCE_RERUN -ne 0 ]; then
    ...
    python -u simulate.py \
           --out $out \
           --size $EXPERIMENT_STREAM_SAMPLES_TEST
fi
